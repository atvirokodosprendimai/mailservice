...
- R7. `ClaimMailbox`'s existing session-reuse check (`paymentSessionReusable`) is corrected — not merely verified — to a Paddle-appropriate mapping: only `draft`/`ready` (Paddle's equivalent of the port's open/pending status) is reusable; `billed`/`paid`/`completed`/`canceled` are not; 404 maps to `ErrPaymentSessionNotFound`. A `draft`/`ready` transaction older than a defined TTL is also treated as not reusable, since Paddle's checkout-URL validity window for an aging `ready` transaction is not guaranteed indefinite.
...
- **KTD5 — Webhook idempotency via both `event_id` dedup and an `occurred_at` ordering guard, applied after mailbox resolution.** A new `last_payment_event_at` column (and a `last_payment_event_id` column) on `mailboxes` are compared against each webhook before applying a state change — `event_id` catches identical redelivery (which the `occurred_at` guard alone cannot, since a retry carries the same timestamp and `RenewMailbox` is additive, not idempotent, on `expires_at`); `occurred_at` catches genuine out-of-order delivery of *different* events. Both checks run **after** the mailbox is resolved from the event (by `subscription_id` or `custom_data.mailbox_id`), not before — the guard's baseline is a per-mailbox column and cannot be evaluated without knowing which mailbox.
...
**Goal:** Fix `paymentSessionReusable` (`internal/core/service/mailbox_service.go:217`) to actually reject completed/canceled sessions — its current implementation only rejects `Expired`/`Failed`, so as written it would hand back an already-completed Paddle checkout URL, reintroducing the exact defect commit `e3e1600` fixed for Polar. This is a **code change**, not verification-only as originally scoped.
...
**Files:**
- `internal/core/service/mailbox_service.go` (`paymentSessionReusable`)
- `internal/core/service/mailbox_service_test.go`
- `internal/adapters/payment/stripe_gateway_test.go` (regression — `paymentSessionReusable` is shared with Stripe, so this fix changes Stripe's reusability behavior too)

**Approach:** Invert `paymentSessionReusable` from a blocklist (reject only `Expired`/`Failed`) to a whitelist: reusable only when `sess.Status` is the port's open/pending status; not reusable for `Succeeded` or any other status; not reusable and a fresh transaction minted on `ErrPaymentSessionNotFound`. Add a TTL check: a `draft`/`ready` session older than a defined age (24h) is also not reusable, since Paddle's checkout-URL validity window for an aging `ready` transaction isn't guaranteed indefinite the way a deliberately-`draft` one might be.
...
**Test scenarios:**
- Existing pending mailbox with a Paddle session in the open/pending status, within the TTL → reused, no new transaction created.
- Existing pending mailbox with a Paddle session in the succeeded status → not reused, fresh transaction minted (this is the case the original unit's "no change needed" premise got wrong).
- Existing pending mailbox with a Paddle session in a failed/canceled status → not reused, fresh transaction minted.
- Existing pending mailbox with a `draft`/`ready` session older than the TTL → not reused, fresh transaction minted.
- Existing pending mailbox whose session returns 404 → not reused, fresh transaction minted.
- Stripe gateway regression: existing Stripe-backed pending mailbox reusability behavior is unchanged by this fix (same shared service-layer function, different gateway).
