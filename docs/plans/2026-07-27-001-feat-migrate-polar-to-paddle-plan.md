...
- **KTD8 — Discount-rejection mapping, RESOLVED 2026-07-27 from Paddle's published error reference.** Paddle's discount error shape is a structured `error.code` on a `400`, not Polar's bare `422`. The exact codes are now enumerated rather than substring-matched: `discount_expired` and `discount_usage_limit_exceeded` map to `ErrCouponExhausted`; `transaction_discount_not_found`, `transaction_discount_not_eligible`, `transaction_invalid_discount_currency`, `discount_restricted_product_not_active`, and `discount_restricted_product_price_not_active` map to `ErrCouponInvalid`. A substring fallback on `discount` remains so any code Paddle adds later still surfaces as a coupon error rather than a generic 500, defaulting to the safer `ErrCouponInvalid` (never falsely claiming the shared gift code is used up). Paddle documents that expired/fully-redeemed discounts *cannot* be redeemed against transactions — so create errors rather than silently omitting the discount — but U7's total-comparison check against the expected discounted amount stays as defense-in-depth for the silent-acceptance branch.
...
- ~~Paddle propagates transaction `custom_data` onto the subscription and onto renewal transactions — unverified until sandbox-tested~~ **RESOLVED 2026-07-27 via Paddle's own documentation** (`developer.paddle.com/build/transactions/custom-data`, retrieved through the Paddle docs MCP): *"Paddle automatically copies transaction `custom_data` to the subscription created from it, and copies subscription `custom_data` to all future transactions (renewals, upgrades, one-time charges)."* The renewal join-key strategy (R4, KTD3, KTD6) is sound as designed — `custom_data.mailbox_id` set at checkout survives onto the subscription and onto every renewal transaction. Note this is **first-party documentation confirmation, not empirical sandbox observation**; Goal Capsule stop condition (c) asked for the latter, so a sandbox run should still confirm it in practice, but the design no longer rests on an unknown and no contingency redesign is needed.
- ~~The exact Paddle discount-rejection error code/type (KTD8) is unknown until sandbox-verified in U7~~ **RESOLVED 2026-07-27** from Paddle's published error reference — exact codes are now enumerated in `mapPaddleDiscountError` with a substring fallback for future additions. See KTD8.
...
- ~~**Discount-rejection error mapping (U7/KTD8) is unverified until sandbox-tested**~~ — **RESOLVED 2026-07-27**: exact error codes confirmed from Paddle's published error reference and enumerated in code (see KTD8). The silent-acceptance branch remains guarded by the total-comparison check. Residual risk is only documented-vs-observed drift, materially lower than the original unknown.
...
- ~~**Whether Paddle propagates `custom_data` onto subscriptions and renewal transactions at all is unverified**~~ — **RESOLVED 2026-07-27**: Paddle's documentation states it explicitly (see Assumptions). The renewal join-key strategy holds. Remaining residual risk is only that documented behavior differs from observed behavior — low, but a sandbox run confirming it is still worth doing as part of stop condition (c).
